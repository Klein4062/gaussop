...
FORMS.append(("1-提出-质量改进诉求单",
    ...
    [
     ...
     ("三、诉求总体内容",[
        ("诉求标题","必填","文本","一句话概括诉求",""),
        ("关联运维系统单号","必填","关联","关联的运维系统单号","运维单号即可,无需另填诉求来源"),
        ("诉求描述","必填","富文本(可粘贴图片)","问题概述、影响、期望达成","支持富文本与粘贴图片"),
     ]),
     ("四、评审人",[
        ("评审人(工号+姓名)","必填","枚举(单选)","评审人(固定名单)","固定下拉名单,由系统配置;表单二评审人继承此项"),
     ]),
    ]))
...
FORMS.append(("2-评审-评审记录单",
    ...
    [
     ("一、评审信息",[
        ...
        ("评审人(工号+姓名)","系统生成","人员(工号+姓名)","评审人工号与姓名(成对)","系统继承提出单填写的评审人"),
     ]),
     ("二、评审结论",[
...
FORMS.append(("3-分析-改进项分析单",
    ...
    [
     ("一、单据信息",[
        ...
        ("责任人(工号+姓名)","系统生成","人员(工号+姓名)","改进项责任人","系统继承自评审阶段填写的责任人"),
     ]),
     ("二、分析与接纳",[
     ...
     ("三、闭环单号(接纳后必填,需校验合法性)",[
        ("问题单号/需求单号","条件必填","编号","闭环对应的单号","是否接纳为『是』时必填;需校验编号合法性(格式正确且单据真实存在),校验通过方可进入闭环阶段"),
     ]),
...
FORMS.append(("4-闭环单",
    ...
    [
     ("一、单据信息",[
        ("闭环方式","系统生成","枚举(单选)","闭环路径","系统继承自分析阶段的闭环方法"),
        ("闭环单号","系统生成","编号","闭环单唯一编号","系统继承自分析阶段的问题单号/需求单号;前缀:PC-问题单/RC-需求"),
        ("关联主诉求单号","必填","关联","所属主诉求单","系统继承"),
        ("当前状态","必填","枚举(单选)","单据状态","默认『实施中』"),
        ("责任人(工号+姓名)","系统生成","人员(工号+姓名)","改进项责任人","系统继承自分析阶段责任人"),
...
FORMS.append(("5-验收-验收单",
    ...
    [
     ...
     ("二、验收结论",[
        ("验收结论","必填","富文本(可粘贴图片)","验收结论说明","支持富文本与粘贴图片;不通过时截图贴入结论"),
        ("验收是否通过","必填","枚举(单选)","是否通过验收","通过/不通过;不通过则打回闭环阶段"),
     ]),
    ]))
