...
OPT={
 ...
 "闭环方法":["问题单闭环","需求闭环"],
 "验收是否通过":["通过","不通过"],
 "评审人":["张三(10001)","李四(10002)","王五(10003)"],   # 固定下拉名单(示例,实际由系统配置)
}
...
merge_val(ws,3,1,6,
    "① 提出  ➜  ② 评审(通过→指派责任人;不通过→打回提出)  ➜  ③ 改进项分析(接纳→定闭环方法;不接纳→打回评审)  ➜  ④ 闭环(闭环方式继承自分析)  ➜  ⑤ 验收(谁提出谁验收,通过即完成)",
    FILL_BOX,F_BOX,C); box(ws,3,1,3,6); ws.row_dimensions[3].height=40
merge_val(ws,4,1,6,"单链流程（不拆子项）；④ 闭环单合一,由闭环方式(问题单闭环/需求闭环)决定单号前缀 PC-/RC-",FILL_SUB,F_NOTE,C)
...
rules=[
 ...
 "4. 改进项分析：接纳后决定闭环方法（问题单/需求）进入闭环；不接纳则打回评审。",
...
rows=[
 ("① 提出","1-提出-质量改进诉求单","记录诉求(关联运维系统单号)","是否受理"),
 ("② 评审","2-评审-评审记录单","评审、指派责任人","通过/不通过(打回提出)"),
 ("③ 分析","3-分析-改进项分析单","接纳与否、定闭环方法","是否接纳(不接纳打回评审)"),
 ("④ 闭环","4-闭环单","由闭环方式决定单号;进展与效果自测","进展/自测/SLA/闭环方式"),
...
render_form(ws1,"质量改进诉求单（提出阶段）",
    ...
    {"title":"三、诉求总体内容","fields":[
        ("SINGLE","诉求标题",26),
        ("SINGLE","关联运维系统单号",26),
        ("SINGLE","诉求描述（富文本,可粘贴图片）",60),
    ]},
    {"title":"四、评审人（固定下拉名单）","fields":[
        ("SINGLE","评审人(工号+姓名)",24,OPT["评审人"]),
    ]},
])
...
heads=["主诉求单号","提出人","关联运维系统单号","评审结果","责任人",
       "分析·是否接纳","闭环方法","SLA时间","验收·是否通过","当前状态"]
widths=[15,12,16,10,12,12,12,12,13,11]
for i,w in enumerate(widths,start=1):
    wsL.column_dimensions[get_column_letter(i)].width=w
merge_val(wsL,1,1,10,"改进诉求台账（中枢 · 汇总各阶段进度）",FILL_TITLE,F_TITLE,C)
wsL.row_dimensions[1].height=32
merge_val(wsL,2,1,10,"一张主诉求单即一项改进措施,单链流转;本表汇总其在各阶段的结果与状态。"
...
samples=[
 ["QI-2026-001","张三/工艺部","YW-2026-007","通过","张工","是","问题单闭环","2026-07-10","通过","已完成"],
 ["QI-2026-002","李四/研发部","YW-2026-019","通过","李工","是","需求闭环","2026-07-20","待验收","待验收"],
 ["QI-2026-003","王五/质量部","YW-2026-031","不通过","—","—","—","—","—","已驳回"],
]
r=4
for row in samples:
...
for _ in range(8):
    ...
    wsL.row_dimensions[r].height=22; r+=1
merge_val(wsL,r+1,1,10,"注：浅色行为示例数据,实际使用时替换或删除。",FILL_SUB,F_NOTE,L)


# ---------------- Sheet 3: 改进项分析 ----------------
ws3=wb.create_sheet("3-分析-改进项分析单")
render_form(ws3,"改进项分析单（分析阶段）",
    "关联：主诉求单号    接纳后进入闭环;不接纳则打回评审",[
    {"title":"一、单据信息","fields":[
        ("TWO",("关联主诉求单号",None,SYS),("分析日期",None,SYS)),
        ("SINGLE","责任人(工号+姓名)",24,None,SYS),
        ("SINGLE","当前状态",22,OPT["状态"],SYS),
    ]},
    {"title":"二、分析与接纳","fields":[
        ("TWO","接纳版本",("是否接纳",OPT["是否"])),
        ("SINGLE","闭环方法",24,OPT["闭环方法"]),
        ("SINGLE","不接纳理由（是否接纳为『否』时必填,否则打回评审）",40),
    ]},
    {"title":"三、分析进展子项（可逐条单独提交,作为外部审视进展的依据）","fields":[
...
# ---------------- Sheet 4: 闭环单(问题单/需求合一) ----------------
ws4=wb.create_sheet("4-闭环单")
render_form(ws4,"闭环单（闭环阶段 · 闭环方式由分析阶段继承）",
    "关联：主诉求单号    闭环方式由分析阶段继承,通过即进入验收",[
    {"title":"一、单据信息","fields":[
        ("SINGLE","闭环方式",24,None,SYS),
        ("SINGLE","关联主诉求单号",24,None,SYS),
        ("TWO",("当前状态",OPT["状态"],SYS),("责任人(工号+姓名)",None,SYS)),
...
render_form(ws5,"验收单（谁提出谁验收,不可转单）",
    "关联：主诉求单号    验收人=改进提出人;不通过则打回闭环(截图贴入验收结论)",[
    {"title":"一、单据信息","fields":[
        ("SINGLE","关联主诉求单号",24,None,SYS),
        ("SINGLE","验收人(工号+姓名)",24,None,SYS),
        ("SINGLE","当前状态",22,OPT["状态"],SYS),
    ]},
    {"title":"二、验收结论","fields":[
...
ids=[
 ("主诉求单号","QI-YYYY-NNN","主单唯一编号,全流程主键(诉求单号贯穿全程)"),
 ("关联运维系统单号","YW-YYYY-NNN","提出单关联的运维系统单号(用户填写)"),
]
for a,b,cc in ids:
...
r+=1
merge_val(ws6,r,1,3,"三、其他约定：评审人为固定下拉名单(系统配置);诉求描述/闭环自测/验收结论支持富文本与粘贴图片;"
