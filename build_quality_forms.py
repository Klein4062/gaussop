...
merge_val(ws,3,1,6,
    "① 提出  ➜  ② 评审(通过→指派责任人;不通过→打回提出)  ➜  ③ 改进项分析(接纳→定闭环方法→挂闭环单号;不接纳→打回评审)  ➜  ④ 闭环(闭环方式继承自分析)  ➜  ⑤ 验收(谁提出谁验收,通过即完成)",
...
render_form(ws4,"闭环单（闭环阶段 · 闭环方式与单号由分析阶段继承）",
    "关联：主诉求单号    闭环方式与单号均由分析阶段继承",[
    {"title":"一、单据信息","fields":[
        ("TWO",("闭环方式",None,SYS),("闭环单号",None,SYS)),
        ("SINGLE","关联主诉求单号",24,None,SYS),
