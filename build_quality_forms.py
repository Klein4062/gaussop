...
widths=[15,12,16,10,12,12,12,13,12,13,11]
for i,w in enumerate(widths,start=1):
    wsL.column_dimensions[get_column_letter(i)].width=w
merge_val(wsL,1,1,11,"改进诉求台账（中枢 · 汇总各阶段进度）",FILL_TITLE,F_TITLE,C)
wsL.row_dimensions[1].height=32
merge_val(wsL,2,1,11,"一张主诉求单即一项改进措施,单链流转;本表汇总其在各阶段的结果与状态。"
...
samples=[
 ["QI-2026-001","张三/工艺部","YW-2026-007","通过","张工","是","问题单闭环","PC-2026-001","2026-07-10","通过","已完成"],
 ["QI-2026-002","李四/研发部","YW-2026-019","通过","李工","是","需求闭环","RC-2026-002","2026-07-20","待验收","待验收"],
...
for _ in range(8):
    ...
    wsL.row_dimensions[r].height=22; r+=1
merge_val(wsL,r+1,1,11,"注：浅色行为示例数据,实际使用时替换或删除。",FILL_SUB,F_NOTE,L)
...
render_form(ws4,"闭环单（闭环阶段 · 由闭环方式决定单号前缀）",
    "关联：主诉求单号 + 分析单号    闭环方法与单号均由分析阶段继承",[
    {"title":"一、单据信息","fields":[
        ("TWO",("闭环方式",None,SYS),("闭环单号",None,SYS)),
        ("TWO",("关联主诉求单号",None,SYS),("关联分析单号",None,SYS)),
...
render_form(ws5,"验收单（谁提出谁验收,不可转单）",
    "关联：主诉求单号 + 闭环单号    验收人=改进提出人;不通过则打回闭环(截图贴入验收结论)",[
    {"title":"一、单据信息","fields":[
...
ids=[
 ...
 ("闭环单号","PC-YYYY-NNN / RC-YYYY-NNN","闭环单编号(系统生成,前缀随闭环方式)"),
